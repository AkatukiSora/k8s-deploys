...
| `app:proxmox:cluster:main:auditor` | `proxmox-auditors`  | `PVEAuditor`              |
...
read-only access は Blueprint 側で `app:proxmox:cluster:main:auditor` を定義済みです。Proxmox 側では `proxmox-auditors` claim に対応する ACL group を作成してください。
