...
Proxmox は例外で、Authentik 内部の `app:proxmox:*` から Proxmox-safe な `proxmox-*` claim へ変換します。
...
- In the first Capsule rollout phase, tenant namespaces must not create `Application` or `ApplicationSet` resources directly
- Tenant `AppProject` objects are platform-managed even when project-scoped roles are used for logs and sync visibility
...
- Cloudflare Access is treated as an edge access layer, not as the source of app-internal RBAC
...
1. Authentik login works from Cloudflare Zero Trust
2. `groups` includes only `app:cloudflare:*`
3. `app:cloudflare:resource:argocd:access` can pass the Argo CD edge policy
4. `app:cloudflare:resource:proxmox:access` can pass the Proxmox edge policy
5. Users without the Cloudflare edge access groups are denied before reaching the upstream app
