...
- Mender uses the private `sora-lab-mender` Backblaze B2 bucket through
  `https://s3.us-west-004.backblazeb2.com` for Artifact storage and device
  Artifact URLs.
...
- Backblaze B2 credentials are supplied through the 1Password-synchronized
  `mender-artifacts-b2` Secret. The legacy RGW ObjectBucketClaim remains
  protected as a rollback path.
...
- Backblaze B2 endpoint, bucket, region, and path-style setting.
...
- Basic public Mender and B2 paths are deployed; production-size Artifact
